...
* node['fb_ssh']['authorized_keys_key_map']
...
1. The contents of `node['fb_ssh']['authorized_principals']` will be used
   to populate `/etc/ssh/authorized_princs/` with one file for each
   user. To limit which users are allowed to have keys at all, simply populate
   the list `node['fb_ssh']['authorized_principals_users']`. The format of the
...
If `node['fb_ssh']['authorized_principals_users']` is not empty, and a user
doesn't exist as a key, no principals will be populated and its file will
be removed.
...
1. Your AuthorizedKeysFile will be set to `/etc/ssh/authorized_keys/%u`,
   regardless of what you set it to
1. The contents of the databag `fb_ssh_authorized_keys`
   will be used to populate `/etc/ssh/authorized_keys/` with key files for each
   user. To limit which users get keys, simply populate the list
   `node['fb_ssh']['authorized_keys_users']`. If
   `node['fb_ssh']['authorized_keys_users']` is not empty, and a user
   doesn't exist as a key, no principals will be populated and its file will
   be removed.

The format of the items in databag is:
...
There should be one item for each user, as many keys as you'd like may be in
that item.
...
```ruby
node.default['fb_ssh']['authorized_keys']['john']['keyname1'] = '...'
```

Any key in the node overrides databags.

You can further limit which keys are installed on a given host by populating the
`authorized_keys_key_map`:

```ruby
# only key 4 goes on this host
node.default['fb_ssh']['authorized_keys_key_map']['john'] = ['keyname4']
```

By default, the keymap is consulted if there's an entry for the given user,
otherwise all keys found are installed.

If you want to _require_ keymaps are always used, you can set `reuqire_key_map`:

```ruby
node.default['fb_ssh']['require_key_map'] = true
```
