...
In this case, we will double-write to a key that matches what we will rename our old cookbook to (`fb_foo_old`).
...
For this, we recommend using the UpdateToplevelNodeAttribute Cookstyle cop in this repo.
...
- role run lists
- resource `cookbook` references (See `cookbook` of https://docs.chef.io/resources/cookbook_file/#properties)
- documentation

For updating `include_recipe` calls we recommend the UpdateIncludeRecipe Cookstyle cop in this repo.

Delete old cookbook
-------------------

Now that all consuming cookbooks and run lists have `fb_foo_old`, you should be able to safely remove `fb_foo`.

Add new cookbook to your repo
-----------------------------

Now that the old `fb_foo` is removed, you can import the new `fb_foo` that you want to pull in.
