...
describe 'Chef::Node' do
      ...
      it 'should handle major versions properly' do
        {
          ...
          expect(node.ubuntu_v?(v)).to eq(r)
          expect(node.ubuntu_v?(v.to_s)).to eq(r)
        end
      end
      ...
      it 'should handle minor versions properly' do
        {
          ...
          expect(node.ubuntu_v?(v)).to eq(r)
          expect(node.ubuntu_v?(v.to_f)).to eq(r)
        end
      end
    end
  end
      ...
      it 'handles versions correctly' do
        {
          ...
          expect(node.fedora_v?(v)).to eq(r)
          expect(node.fedora_v?(v.to_s)).to eq(r)
        end
      end
    end
  end
  ...
  context 'CentOS 9' do
    ...
    context 'Chef::Node.centos_v?' do
      it 'handles versions correctly' do
        expect(node.centos_v?(8)).to eq(false)
        expect(node.centos_v?(9)).to eq(true)
        expect(node.centos_v?(10)).to eq(false)

        expect(node.centos_v?(9.1)).to eq(false)
        expect(node.centos_v?(9.2)).to eq(false)
      end
    end
  end
      ...
      it 'handles versions correctly' do
        {
          ...
          expect(node.centos_v?(v)).to eq(r)
          expect(node.centos_v?(v.to_s)).to eq(r)
        end
      end
    end
  end
  ...
  context 'Arista EOS 4.31' do
    ...
    context 'Chef::Node.aristaeos_v_plus?' do
      it 'handles versions correctly' do
        {
          '4.28' => true,
          '4.30' => true,
          ...
          expect(node.aristaeos_v_plus?(v)).to eq(r)
        end
      end
    end
  end
