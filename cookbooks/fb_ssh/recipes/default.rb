...
execute 'generate host keys if they do not exist' do
  not_if { Dir.glob(::File.join(FB::SSH.confdir(node), '*key')).any? }
  command 'ssh-keygen -A'
end

# sshd won't start if the private keys are too readable, this will fix
# them appropriately
fb_ssh_hostkey_perms 'doit'
...
fb_ssh_clean_confd_dirs 'sshd' do
  action :clean_sshd_d
  notifies :restart, 'service[ssh]', ntype
end
...
fb_ssh_clean_confd_dirs 'ssh' do
  action :clean_ssh_d
end
