...
recipe 'fb_mlocate::packages', :unsupported => [:mac_os_x] do |tc|
  let(:chef_run) do
    tc.chef_run
  end

  context 'when want_mlocate is true' do
    it 'upgrades the mlocate package' do
      chef_run.converge(described_recipe) do |node|
        node.default['fb_mlocate']['want_mlocate'] = true
      end
      expect(chef_run).to upgrade_package('mlocate')
    end
  end
  ...
  context 'when want_mlocate is false' do
    it 'does not upgrade the mlocate package' do
      chef_run.converge(described_recipe) do |node|
        node.default['fb_mlocate']['want_mlocate'] = false
      end
